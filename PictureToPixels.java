...
public class PictureToPixels extends Application {
    ...
    public void start(Stage theStage) {
        stage = theStage;
        // Calls drawGUI so it can be redrawn instead of creating new windows by calling
        // start
        drawGUI();
    }
    ...
    private void photoChoice(ActionEvent event) {
        try {
            ...
            if (f != null) {
                ...
                // callse imgToWritable which converts the image to a writable image so it can
                // be editted
                imgToWritable(img);
            }
    ...
    private void imgToWritable(Image img) {
        // Sets the global writable image to a new writable image with the same
        // dimensions as the image sent
        wImage = new WritableImage((int) img.getWidth(), (int) img.getHeight());
